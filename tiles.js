...
const {parsePng,bitmapFrom,readIndexedPixels}=window.TilePng;
const Lang=window.TileLang,tr=(k,v)=>Lang?Lang.t(k,v):k;
const {parsePaletteImport,rgbToSms,smsToRgb,hex,hex2,paletteRows}=window.PaletteIO;
...
const state={
  ...
  hover:-1,reorder:false,drag:null,sourceScale:1,tileLayout:null,leftW:260,rightW:360,
  paletteOrder:[],palCounts:new Map(),palHover:-1,palReorder:false,palEdit:false,palDrag:null,palLayout:null,palTarget:-1,palOriginal:null,palVariants:[],activePal:0,
...
const ui={
  ...
  palCanvas:$('paletteCanvas'),palEmpty:$('paletteEmpty'),palReorder:$('paletteReorderBtn'),palEdit:$('paletteEditBtn'),palImport:$('paletteImportBtn'),palImportInput:$('paletteImportInput'),palDefault:$('paletteDefaultBtn'),palVariant:$('paletteVariantBtn'),palVariantInput:$('paletteVariantInput'),palVariants:$('paletteVariants'),
  picker:$('palPicker'),pickerBox:$('palPickerBox'),pickerTitle:$('palPickerTitle'),pickerCurrent:$('palPickerCurrent'),pickerClose:$('palPickerClose'),pickerCancel:$('palPickerCancel'),smsGrid:$('smsGrid'),
  exportPng:$('exportPng'),exportBin:$('exportBin'),exportPsg:$('exportPsg'),exportZx7:$('exportZx7'),exportTsx:$('exportTsx'),
  exportTmBin:$('exportTmBin'),exportTmAsm:$('exportTmAsm'),exportTmRle:$('exportTmRle'),exportTmStm:$('exportTmStm'),exportTmTmx:$('exportTmTmx'),
  exportPalBin:$('exportPalBin'),exportPalAsm:$('exportPalAsm'),exportPalAct:$('exportPalAct'),exportPalGpl:$('exportPalGpl')
};
...
if(Lang){Lang.init();Lang.onchange=()=>{buildSmsGrid();updateStats();renderSource();renderTiles();renderPalette();renderEditCanvas();if(state.palTarget>=0)updatePickerInfo()}}
bindUi();
buildSmsGrid();
initSplitters();
updateGrid();
...
async function loadFile(file){
  try{
    ...
    if(png.colorType!==3)throw Error(tr('png.err.indexed'));
    if(!png.palette||!png.palette.length)throw Error(tr('png.err.plte'));
    if(png.palette.length>16)throw Error(tr('png.err.colors'));
    if(png.width%8||png.height%8)throw Error(tr('png.err.size'));
    const bmp=await bitmapFrom(buf,file),pixels=readIndexedPixels(bmp,png),pal=png.palette.map(p=>({r:p.r,g:p.g,b:p.b}));
    state.src={w:png.width,h:png.height,palette:pal,defaultPalette:clonePalette(pal),pixels,bitmap:bmp};
    state.palVariants=[{name:'PNG',colors:pal,base:true}];state.activePal=0;
    setUsedPalette(pixels,pal.length);
    state.name=file.name.replace(/\.png$/i,'')||'tiles';
    state.fileName=file.name;state.fileW=png.width;state.fileH=png.height;state.fileColors=png.palette.length;
...
function resetAfterError(msg){
  state.src=null;state.tiles=[];state.tilemap=null;state.raw=0;state.removed=0;state.hover=-1;state.reorder=false;state.drag=null;resetTileEditState();resetGeneratedStats();
  state.paletteOrder=[];state.palCounts=new Map();state.palHover=-1;state.palReorder=false;state.palEdit=false;state.palDrag=null;state.palVariants=[];state.activePal=0;
  ...
  for(let y=0,ty=0;y<src.h;y+=8,ty++)for(let x=0,tx=0;x<src.w;x+=8,tx++){
    ...
    if(ui.dedupe.checked){
      ...
      if(old){
        ...
        const tile={data,occ:1,positions:[{...pos,h:false,v:false}]};
        tiles.push(tile);
...
function updateStats(){
  ...
  ui.exportPng.disabled=!n;ui.exportBin.disabled=!n;ui.exportPsg.disabled=!n||typeof window.compressTilesPSG!=='function';ui.exportZx7.disabled=!n||typeof window.compressZX7!=='function';ui.exportTsx.disabled=!n;ui.reorder.disabled=!n||state.edit.active;ui.editBtn.disabled=!n;
  ui.exportTmBin.disabled=!mapCount;ui.exportTmAsm.disabled=!mapCount;
  ...
  ui.palReorder.disabled=!state.paletteOrder.length||state.edit.active;ui.palEdit.disabled=!state.paletteOrder.length||state.edit.active;ui.palImport.disabled=!state.paletteOrder.length||state.edit.active;if(ui.palVariant)ui.palVariant.disabled=!state.paletteOrder.length||state.edit.active;
  const pd=paletteDirty();
  ui.palDefault.disabled=!pd;ui.palDefault.classList.toggle('is-hidden',!pd);
  ui.exportPalBin.disabled=!state.paletteOrder.length;ui.exportPalAsm.disabled=!state.paletteOrder.length;ui.exportPalAct.disabled=!state.paletteOrder.length;ui.exportPalGpl.disabled=!state.paletteOrder.length;
...
function updateEditColorUi(){
  ...
  const sms=rgbToSms(p);ui.editSwatch.style.background=hex(p);ui.editColorInfo.textContent=tr('edit.color',{idx:state.edit.selectedPal,hex:hex(p),sms:hex2(sms.v)});
...
function highlightSource(x,scale){
  ...
  if(!tile)return;
  tile.positions.forEach((p,i)=>{const sx=Math.round(p.x*scale),sy=Math.round(p.y*scale),sw=Math.max(1,Math.round(8*scale)),sh=Math.max(1,Math.round(8*scale));x.fillStyle=i?'rgba(240,160,48,.20)':'rgba(74,158,255,.30)';x.fillRect(sx,sy,sw,sh);x.lineWidth=Math.max(1,Math.round(scale>=1?1:0));x.strokeStyle=i?'rgba(240,160,48,.95)':'rgba(74,158,255,1)';x.strokeRect(sx+.5,sy+.5,Math.max(1,sw-1),Math.max(1,sh-1))});
}
function renderTiles(){
  const src=state.src,c=ui.tiles,x=c.getContext('2d'),tiles=state.tiles;
  if(!src||!tiles.length){c.width=1;c.height=1;state.tileLayout=null;ui.tilesEmpty.style.display='block';updateStats();return}
  ui.tilesEmpty.style.display='none';
  const scale=3,gap=0,tileSize=8*scale,cols=TILE_SHELF_COLS,rows=Math.ceil(tiles.length/cols);
...
function renderPalette(){
  ...
  state.palLayout={w,h,gap,cols,rows};c.width=cols*w+(cols-1)*gap;c.height=h;x.clearRect(0,0,c.width,c.height);
  for(let i=0;i<cols;i++){const palIdx=order[i],ox=i*(w+gap),oy=0,dragged=state.palDrag&&state.palDrag.from===i,target=state.palDrag&&state.palDrag.to===i;x.strokeStyle='rgba(255,255,255,.12)';x.strokeRect(ox+.5,oy+.5,w-1,h-1);if(palIdx===undefined)continue;const p=src.palette[palIdx];x.save();if(dragged)x.globalAlpha=.35;x.fillStyle=`rgb(${p.r},${p.g},${p.b})`;x.fillRect(ox,oy,w,h);x.restore();if(i===state.palHover||target||dragged||palIdx===state.edit.selectedPal&&state.edit.active&&state.edit.index>=0){x.strokeStyle=palIdx===state.edit.selectedPal&&state.edit.active&&state.edit.index>=0?'rgba(0,200,104,1)':dragged?'rgba(240,160,48,1)':target?'rgba(0,200,104,1)':'rgba(74,158,255,1)';x.lineWidth=2;x.strokeRect(ox+1,oy+1,w-2,h-2)}}
...
function onPaletteClick(e){const idx=paletteAt(e);if(idx<0)return;if(state.edit.active&&state.edit.index>=0){state.edit.selectedPal=state.paletteOrder[idx];clearTooltip();renderPalette();renderEditCanvas();updateEditColorUi();return}if(!state.palEdit||state.palDrag)return;openPicker(idx)}
function movePalette(from,to){const a=state.paletteOrder,p=a.splice(from,1)[0];a.splice(to,0,p)}
...
function showPaletteTip(e,idx){const palIdx=state.paletteOrder[idx],p=state.src.palette[palIdx],sms=rgbToSms(p);tip.textContent=tr('tip.palette',{idx,png:palIdx,hex:hex(p),sms:hex2(sms.v),r:sms.r,g:sms.g,b:sms.b,occ:state.palCounts.get(palIdx)||0});placeTip(e)}
function showPaletteDragTip(e,from,to){const a=state.paletteOrder,p=a[from],sms=rgbToSms(state.src.palette[p]);tip.textContent=tr('tip.dragColor',{from,to,png:p,sms:hex2(sms.v)});placeTip(e)}
...
function clearHover(){if(state.hover!==-1){state.hover=-1;renderSource();renderTiles()}clearTooltip()}
...
function openPicker(idx){const palIdx=state.paletteOrder[idx],p=state.src.palette[palIdx];state.palTarget=palIdx;state.palOriginal={r:p.r,g:p.g,b:p.b};ui.pickerTitle.textContent=tr('picker.titleFull',{idx,png:palIdx});ui.picker.hidden=false;centerPicker();updatePickerInfo();clearTooltip()}
function closePicker(){ui.picker.hidden=true;state.palTarget=-1;state.palOriginal=null}
function cancelPicker(){if(state.src&&state.palTarget>=0&&state.palOriginal){state.src.palette[state.palTarget]={...state.palOriginal};renderSource();renderTiles();renderPalette();updateStats()}closePicker()}
function pickSms(v){if(!state.src||state.palTarget<0)return;const p=smsToRgb(v);state.src.palette[state.palTarget]={r:p.r,g:p.g,b:p.b};renderSource();renderTiles();renderPalette();updateStats();updatePickerInfo();toast(tr('toast.colorChanged',{sms:hex2(v)}))}
function updatePickerInfo(){if(!state.src||state.palTarget<0)return;const p=state.src.palette[state.palTarget],sms=rgbToSms(p),o=state.palOriginal,os=o?`<span class="palette-origin">${xml(tr('picker.origin',{hex:hex(o)}))}</span>`:'';ui.pickerCurrent.innerHTML=`<span class="palette-current-swatch" style="background:${hex(p)}"></span><span>${xml(tr('picker.current',{hex:hex(p),sms:hex2(sms.v),r:sms.r,g:sms.g,b:sms.b}))}</span>${os}`;ui.smsGrid.querySelectorAll('.sms-cell').forEach(b=>b.classList.toggle('sel',Number(b.dataset.v)===sms.v))}
...
function renderPaletteVariants(){const box=ui.palVariants;if(!box)return;box.innerHTML='';if(!state.src||!state.paletteOrder.length||!state.palVariants.length){box.style.display='none';return}box.style.display='flex';const order=state.paletteOrder,w=14,h=28,gap=2,cols=16;state.palVariants.forEach((v,idx)=>{const row=document.createElement('div'),name=document.createElement('span'),slot=document.createElement('span');row.className='palette-variant'+(idx===state.activePal?' active':'');row.onclick=e=>{if(!state.edit.active&&idx!==state.activePal&&!e.target.closest('button'))setActivePalette(idx)};name.className='palette-variant-name';name.textContent=v.name||tr('variant.defaultName',{idx:idx+1});slot.className='palette-remove-space';row.appendChild(name);if(idx===state.activePal)row.appendChild(ui.palCanvas);else{const can=document.createElement('canvas'),x=can.getContext('2d');can.width=cols*w+(cols-1)*gap;can.height=h;for(let i=0;i<cols;i++){const palIdx=order[i],ox=i*(w+gap);x.strokeStyle='rgba(255,255,255,.12)';x.strokeRect(ox+.5,.5,w-1,h-1);if(palIdx===undefined)continue;const p=v.colors[palIdx]||{r:0,g:0,b:0};x.fillStyle=`rgb(${p.r},${p.g},${p.b})`;x.fillRect(ox,0,w,h)}row.appendChild(can)}if(idx!==state.activePal){const b=document.createElement('button');b.type='button';b.className='palette-remove';b.textContent='×';b.title=tr('variant.remove');b.onclick=e=>{e.stopPropagation();removePaletteVariant(idx)};row.appendChild(b)}else row.appendChild(slot);box.appendChild(row)})}
function setActivePalette(idx){if(!state.src||!state.palVariants[idx])return;state.activePal=idx;state.src.palette=state.palVariants[idx].colors;state.palHover=-1;closePicker();renderSource();renderTiles();renderPalette();updateStats();toast(tr('toast.paletteActive',{name:state.palVariants[idx].name}))}
...
function exportPaletteBin(){download(`${state.name}_palette.bin`,paletteSmsBytes(),'application/octet-stream')}
function exportPaletteAsm(){const vals=[...paletteSmsBytes()],lines=paletteRows(vals).map(r=>'.db '+r.map(v=>'$'+hex2(v)).join(' ')),txt=`${tr('export.paletteAsmComment',{name:state.name})}\n${lines.join('\n')}\n`;download(`${state.name}_palette.asm`,txt,'text/plain')}
function exportPaletteAct(){const pal=currentPalette(),out=new Uint8Array(768);pal.forEach((p,i)=>{const j=i*3;out[j]=p.r;out[j+1]=p.g;out[j+2]=p.b});download(`${state.name}_palette.act`,out,'application/octet-stream')}
function exportPaletteGpl(){const pal=currentPalette(),lines=['GIMP Palette',`Name: ${state.name}`,`Columns: ${Math.min(16,Math.max(1,pal.length))}`,'#'];pal.forEach((p,i)=>{const sms=rgbToSms(p);lines.push(`${String(p.r).padStart(3,' ')} ${String(p.g).padStart(3,' ')} ${String(p.b).padStart(3,' ')}\t${String(i).padStart(2,'0')} SMS_$${hex2(sms.v)}`)});download(`${state.name}_palette.gpl`,lines.join('\n')+'\n','text/plain')}
